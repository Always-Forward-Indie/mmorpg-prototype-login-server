...
**Unified DB tool — `scripts/db.sh`.**
- `dump_db.sh` и `load_dump.sh` заменены на единый `scripts/db.sh` с интерактивным меню и подкомандами.
- Новые команды: `load-safe` (заливка контента с сохранением игроков и статистики), `backup-players` (выгрузка только данных игроков).
- `load-safe` делает backup 24 таблиц игроков, дропает БД, заливает дамп, восстанавливает игроков и сбрасывает 9 сиквенсов.
